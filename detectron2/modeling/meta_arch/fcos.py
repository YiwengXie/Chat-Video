...
class FCOS(DenseDetector):
    ...
    def forward_training(self, images, features, predictions, gt_instances):
        ...
        (
            ...
            pred_anchor_deltas,
            pred_centerness,
        ) = self._transpose_dense_predictions(
    ...
    def label_anchors(
        ...
        for inst in gt_instances:
            if len(inst) > 0:
                ...
                matched_gt_boxes_i = inst.gt_boxes.tensor[
    ...
    def losses(
        ...
        normalizer = self._ema_update(
            "loss_normalizer", max(num_pos_anchors, 1), 300
        )

        # classification and regression loss
        gt_labels_target = F.one_hot(
            gt_labels, num_classes=self.num_classes + 1
        )[
            :, :, :-1
        ...
        ctrness_targets = self.compute_ctrness_targets(
            anchors, gt_boxes
        )  # (M, R)
        pred_centerness = torch.cat(pred_centerness, dim=1).squeeze(
            dim=2
        )  # (M, R)
    ...
    def compute_ctrness_targets(
        ...
        reg_targets = [
            self.box2box_transform.get_deltas(anchors, m) for m in gt_boxes
        ]
...
class FCOSHead(RetinaNetHead):
    ...
    def __init__(
        self, *, input_shape: List[ShapeSpec], conv_dims: List[int], **kwargs
    ):
        super().__init__(
        ...
        self.ctrness = nn.Conv2d(
            conv_dims[-1], 1, kernel_size=3, stride=1, padding=1
        )
