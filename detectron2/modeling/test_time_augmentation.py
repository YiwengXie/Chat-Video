...
class DatasetMapperTTA:
    ...
    def __call__(self, dataset_dict):
        ...
        if shape[:2] != orig_shape:
            ...
            pre_tfm = ResizeTransform(
                orig_shape[0], orig_shape[1], shape[0], shape[1]
            )
        else:
...
class GeneralizedRCNNWithTTA(nn.Module):
    ...
    def __init__(self, cfg, model, tta_mapper=None, batch_size=3):
        ...
        self.cfg = cfg.clone()
        assert (
            not self.cfg.MODEL.KEYPOINT_ON
    ...
    def _batch_inference(self, batched_inputs, detected_instances=None):
        ...
        outputs = []
        inputs, instances = [], []
        for idx, input, instance in zip(
            count(), batched_inputs, detected_instances
        ):
            inputs.append(input)
    ...
    def __call__(self, batched_inputs):
        ...
        def _maybe_read_image(dataset_dict):
            ...
            if "image" not in ret:
                image = read_image(
                    ret.pop("file_name"), self.model.input_format
                )
    ...
    def _inference_one_image(self, input):
        ...
        if self.cfg.MODEL.MASK_ON:
            ...
            outputs = self._batch_inference(
                augmented_inputs, augmented_instances
            )
            # Delete now useless variables to avoid being out of memory
            del augmented_inputs, augmented_instances
            # average the predictions
            merged_instances.pred_masks = self._reduce_pred_masks(outputs, tfms)
            merged_instances = detector_postprocess(
                merged_instances, *orig_shape
            )
