...
def load_sem_seg(gt_root, image_root, gt_ext="png", image_ext="jpg"):
    ...
    if len(input_files) != len(gt_files):
        ...
        logger.warn(
            "Will use their intersection of {} files.".format(len(intersect))
        )
        input_files = [
...
def convert_to_coco_dict(dataset_name):
    ...
    for image_id, image_dict in enumerate(dataset_dicts):
        ...
        for annotation in anns_per_image:
            ...
            to_bbox_mode = (
                BoxMode.XYWH_ABS if len(bbox) == 4 else BoxMode.XYWHA_ABS
            )
            ...
            if "segmentation" in annotation:
                ...
                if to_bbox_mode == BoxMode.XYWH_ABS:
                    bbox_xy = BoxMode.convert(
                        bbox, to_bbox_mode, BoxMode.XYXY_ABS
                    )
...
def register_coco_instances(
    name, metadata, json_file, image_root, dataset_name_in_dict="coco"
):
    """
