...
class ResizeScale(Augmentation):
    ...
    def _get_resize(self, image: np.ndarray, scale: float) -> Transform:
        ...
        output_scale = np.minimum(
            target_scale_size[0] / input_size[0],
            target_scale_size[1] / input_size[1],
        )
        output_size = np.round(np.multiply(input_size, output_scale)).astype(
            int
        )
        ...
        if self.single_category_max_area >= 1.0:
            ...
            for _ in range(10):
                ...
                if (
                    len(cnt) > 1
                    and np.max(cnt)
                    < np.sum(cnt) * self.single_category_max_area
                ):
                    break
...
class RandomContrast(Augmentation):
    ...
    def get_transform(self, image):
        ...
        return BlendTransform(
            src_image=image.mean(), src_weight=1 - w, dst_weight=w
        )
...
class RandomSaturation(Augmentation):
    ...
    def get_transform(self, image):
        ...
        return BlendTransform(
            src_image=grayscale, src_weight=1 - w, dst_weight=w
        )
