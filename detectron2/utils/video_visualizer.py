...
class VideoVisualizer:
    def __init__(self, metadata, instance_mode=ColorMode.IMAGE):
        ...
        self._color_pool = random_colors(
            self._max_num_instances, rgb=True, maximum=1
        )
    ...
    def draw_instance_predictions(self, frame, predictions):
        ...
        colors = (
            predictions.COLOR
            if predictions.has("COLOR")
            else [None] * len(predictions)
        )
        periods = (
            predictions.ID_period if predictions.has("ID_period") else None
        )
    ...
    def draw_panoptic_seg_predictions(
        ...
        masks_rles = mask_util.encode(
            np.asarray(
                np.asarray(masks).transpose(1, 2, 0), dtype=np.uint8, order="F"
            )
        )
        assert len(masks_rles) == num_instances
    ...
    def _assign_colors(self, instances):
        ...
        if len(ious) == 0:
            ious = np.zeros(
                (len(self._old_instances), len(instances)), dtype="float32"
            )
