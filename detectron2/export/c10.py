...
class Caffe2ROIPooler(Caffe2Compatible, poolers.ROIPooler):
    ...
    def forward(self, x, box_lists):
        ...
        roi_feat_fpn_list = []
        for roi_fpn, x_level, pooler in zip(
            rois_fpn_list, x, self.level_poolers
        ):
        ...
        assert (
            roi_feat_shuffled.numel() > 0 and rois_idx_restore_int32.numel() > 0
        ), (
...
class Caffe2FastRCNNOutputsInference:
    def __init__(self, tensor_mode):
        self.tensor_mode = (
            tensor_mode  # whether the output is caffe2 tensor mode
        )
    ...
    def __call__(self, box_predictor, predictions, proposals):
        ...
        input_tensor_mode = (
            proposals[0].proposal_boxes.tensor.shape[1] == box_dim + 1
        )
