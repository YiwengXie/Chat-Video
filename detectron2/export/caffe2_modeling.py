...
def assemble_rcnn_outputs_by_name(
    image_sizes, tensor_outputs, force_mask_on=False
):
    """
...
def convert_batched_inputs_to_c2_format(
    batched_inputs, size_divisibility, device
):
    """
...
class Caffe2MetaArch(Caffe2Compatible, torch.nn.Module):
    ...
    def _caffe2_preprocess_image(self, inputs):
        ...
        mean, std = (
            self._wrapped_model.pixel_mean,
            self._wrapped_model.pixel_std,
        )
...
class Caffe2GeneralizedRCNN(Caffe2MetaArch):
    ...
    def encode_additional_info(self, predict_net, init_net):
        ...
        check_set_pb_arg(
            predict_net, "size_divisibility", "i", size_divisibility
        )
        check_set_pb_arg(
            predict_net,
            "device",
        ...
        check_set_pb_arg(
            predict_net, "meta_architecture", "s", b"GeneralizedRCNN"
        )
...
class Caffe2RetinaNet(Caffe2MetaArch):
    ...
    def forward(self, inputs):
        ...
        for i, feature_i in enumerate(features):
            features[i] = alias(
                feature_i, "feature_{}".format(i), is_backward=True
            )
    ...
    def _encode_anchor_generator_cfg(self, predict_net):
        ...
        torch.save(
            self._wrapped_model.anchor_generator, serialized_anchor_generator
        )
    ...
    def get_outputs_converter(predict_net, init_net):
        ...
        bbox_reg_weights = get_pb_arg_floats(
            predict_net, "bbox_reg_weights", None
        )
        self.box2box_transform = Box2BoxTransform(
            weights=tuple(bbox_reg_weights)
        )
        self.test_score_thresh = get_pb_arg_valf(
            predict_net, "score_threshold", None
        )
        self.test_topk_candidates = get_pb_arg_vali(
            predict_net, "topk_candidates", None
        )
        self.test_nms_thresh = get_pb_arg_valf(
            predict_net, "nms_threshold", None
        )
        ...
        for meth in [
            ...
            setattr(
                self,
                meth,
                functools.partial(getattr(meta_arch.RetinaNet, meth), self),
            )
