...
def _op_stats(net_def):
    ...
    type_count_list = sorted(
        type_count.items(), key=lambda kv: kv[0]
    )  # alphabet
    type_count_list = sorted(type_count_list, key=lambda kv: -kv[1])  # count
    return "\n".join(
        "{:>4}x {}".format(count, name) for name, count in type_count_list
    )
...
def _assign_device_option(
    ...
    _assign_op_device_option(
        predict_net, predict_net_ssa, predict_net_device_types
    )
...
def export_caffe2_detection_model(
    ...
    table = tabulate(
        ops_table, headers=["type", "input", "output"], tablefmt="pipe"
    )
    logger.info(
...
def run_and_save_graph(predict_net, init_net, tensor_inputs, graph_save_path):
    ...
    logger.info(
        "Saving graph of ONNX exported model to {} ...".format(graph_save_path)
    )
    ...
    with ScopedWS("__ws_tmp__", True) as ws:
        ...
        uninitialized = [
            inp
            for inp in predict_net.external_input
            if inp not in initialized_blobs
        ]
        ...
        logger.info(
            "Saving graph with blob shapes to {} ...".format(graph_save_path)
        )
        save_graph(
            predict_net, graph_save_path, op_only=False, blob_sizes=blob_sizes
        )
