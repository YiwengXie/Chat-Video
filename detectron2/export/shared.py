...
def check_set_pb_arg(pb, arg_name, arg_attr, arg_value, allow_override=False):
    ...
    if allow_override and getattr(arg, arg_attr) != arg_value:
        ...
        ), "Existing value {}, new value {}".format(
            getattr(arg, arg_attr), arg_value
        )
...
def _create_const_fill_op_from_numpy(name, tensor, device_option=None):
    ...
    return core.CreateOperator(
        kTypeNameMapper[tensor.dtype], [], [name], **args_dict
    )
...
def get_params_from_init_net(
    ...
    with ScopedWS(
        "__get_params_from_init_net__", is_reset=True, is_cleanup=True
    ) as ws:
...
def _generic_status_identifier(
    ...
    versioned_ext_output = [
        (b, versions[b]) for b in predict_net.external_output
    ]
...
def infer_device_type(
    ...
    return _generic_status_identifier(
        predict_net, _device_updater, known_status
    )
...
def _get_dependency_chain(ssa, versioned_target, versioned_source):
    ...
    paths = dag.get_all_paths(
        versioned_source, versioned_target
    )  # include two ends
    ops_in_paths = [
        [producer_map[blob][0] for blob in path[1:]] for path in paths
    ]
    ...
    def _fuse_once(predict_net):
        ...
        for op_id, op in enumerate(predict_net.op):
            if op.type in _COPY_OPS:
                ...
                is_fusable = (
                    len(consumer_ids) > 0
                    and fw_copy_versioned_output
                    not in versioned_external_output
                    and all(
