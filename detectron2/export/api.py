...
class Caffe2Model(nn.Module):
    ...
    def save_protobuf(self, output_dir):
        ...
        with PathManager.open(
            os.path.join(output_dir, "model.pbtxt"), "w"
        ) as f:
            f.write(str(self._predict_net))
        with PathManager.open(
            os.path.join(output_dir, "model_init.pb"), "wb"
        ) as f:
    ...
    def save_graph(self, output_file, inputs=None):
        ...
        if inputs is None:
            ...
            device = get_pb_arg_vals(
                self._predict_net, "device", b"cpu"
            ).decode("ascii")
            ...
            run_and_save_graph(
                self._predict_net, self._init_net, inputs, output_file
            )
    ...
    def __call__(self, inputs):
        ...
        if self._predictor is None:
            self._predictor = ProtobufDetectionModel(
                self._predict_net, self._init_net
            )
