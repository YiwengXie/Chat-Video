...
def patch_instances(fields):
    ...
    with tempfile.TemporaryDirectory(
        ...
        try:
            ...
            Instances._jit_override_qualname = (
                torch._jit_internal._qualified_name(new_instances)
            )
...
def patch_builtin_len(modules=()):
    ...
    with ExitStack() as stack:
        ...
        ctxs = [
            stack.enter_context(mock.patch(mod + ".len")) for mod in MODULES
        ]
        for m in ctxs:
...
def patch_nonscriptable_classes():
    ...
    if hasattr(StandardROIHeads, "__annotations__"):
        ...
        StandardROIHeads.__annotations__ = deepcopy(
            StandardROIHeads.__annotations__
        )
