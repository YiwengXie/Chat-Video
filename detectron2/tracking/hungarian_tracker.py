...
class BaseHungarianTracker(BaseTracker):
    ...
    def update(self, instances: Instances) -> Instances:
        ...
        if self._prev_instances is not None:
            ...
            cost_matrix = self.build_cost_matrix(
                instances, self._prev_instances
            )
            ...
            instances = self._process_unmatched_prev_idx(
                instances, matched_prev_idx
            )
    ...
    def _process_matched_idx(
        ...
        for i in range(matched_idx.size):
            instances.ID[matched_idx[i]] = self._prev_instances.ID[
                matched_prev_idx[i]
            ]
    ...
    def _process_unmatched_prev_idx(
        ...
        for idx in untracked_prev_idx:
            ...
            if (
                (
                    1.0 * (x_right - x_left) / self._video_width
                    < self._min_box_rel_dim
                )
                or (
                    1.0 * (y_bot - y_top) / self._video_height
                    < self._min_box_rel_dim
                )
