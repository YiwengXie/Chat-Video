...
class UNINEXT_IMG(nn.Module):
    ...
    def forward(self, batched_inputs, do_postprocess=True):
        ...
        if self.training:
            if self.boxinst_enabled:
                images, targets = self.prepare_image_targets_boxinst(
                    batched_inputs
                )
            else:
                images = self.preprocess_image(batched_inputs)
            ...
            if self.parallel_det:
                language_dict_features = self.forward_text(
                    captions, device="cuda", task=task
                )
            else:
                language_dict_features = self.forward_text(
                    captions, device="cuda"
                )
            output, loss_dict = self.detr.coco_forward(
                images,
                targets,
                self.criterion,
                train=True,
            ...
            output, loss_dict = self.detr.coco_inference(
                images,
                None,
                self.criterion,
                train=False,
    ...
    def prepare_targets(self, targets):
        ...
        for targets_per_image in targets:
            ...
            positive_map = (
                targets_per_image.positive_map
            )  # (N, 256) or (1, 1). N is number of objects per image
            if self.use_amp:
    ...
    def inference(
        ...
        for i, (
            ...
            image_size,
            iou_per_image,
        ) in enumerate(zip(box_cls, box_pred, image_sizes, iou_pred)):

            if self.ota:
                # NMS
                logits_per_image = convert_grounding_to_od_logits(
                    logits_per_image.unsqueeze(0),
                    num_classes,
                    positive_map_label_to_token,
                )
            ...
            if self.mask_on:
                ...
                mask = F.interpolate(
                    mask_pred_i,
                    size=(H * self.mask_stride, W * self.mask_stride),
                    mode="bilinear",
                    align_corners=False,
                )
    ...
    def prepare_image_targets_boxinst(
        self, batched_inputs, size_divisibility=32
    ):
        original_images = [
        ...
        original_images = ImageList.from_tensors(
            original_images, size_divisibility
        )
    ...
    def add_bitmasks_from_boxes(
        ...
        downsampled_images = F.avg_pool2d(
            ...
            :, [2, 1, 0]
        ]  # RGB-format original images (with padding) (bs, 3, H//4, W//4)
        ...
        for im_i, per_im_gt_inst in enumerate(instances):
            ...
            per_im_gt_inst.image_color_similarity = torch.cat(
                [
                    ...
                    for _ in range(len(per_im_gt_inst))  # (N, 8, H//4, W//4)
                ],
                dim=0,
            )
