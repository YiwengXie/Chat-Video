...
class DetrDatasetMapperUni:
    ...
    def __init__(self, cfg, is_train=True, test_categories=None):
        ...
        if self.lang_guide_det:
            ...
            self.ind_to_class_dict["obj365v2"] = cat2ind(
                OBJECTS365V2_CATEGORIES
            )
            use_roberta = (
                ...
                for dataset_name in cfg.DATASETS.TRAIN:
                    if dataset_name.startswith("coco"):
                        ...
                        (
                            ...
                            OBJECTS365V2_CATEGORIES, self.tokenizer
                        )
    ...
    def transform_annos(
        self, annotations_ori, transforms, image_shape, dataset_dict
    ):
        ...
        if self.lang_guide_det and task == "detection":
            ...
            instances, positive_caption_length = check_for_positive_overflow(
                instances, ind_to_class, self.tokenizer, self.max_query_len - 2
            )
    ...
    def __call__(self, dataset_dict):
        ...
        disable_crop = (
            self.has_ordinal_num(dataset_dict["expressions"])
            if "expressions" in dataset_dict
            else False
        )
        dataset_dict["image"], image_shape, transforms = self.transform_img(
            image, disable_crop=disable_crop
        )
        if (
            "expressions" in dataset_dict
            and dataset_dict["task"] == "grounding"
        ):
            dataset_dict["expressions"] = self.transform_expressions(
        ...
        if not self.is_train:
            ...
            task = dataset_dict["task"] if "task" in dataset_dict else None
            if self.lang_guide_det and task == "detection":
                dataset_dict["expressions"] = self.prompt_test_dict[
        ...
        if "annotations" in dataset_dict:
            instances, expressions_new = self.transform_annos(
...
class DetrDatasetMapperUniCLIP(DetrDatasetMapperUni):
    def __init__(self, cfg, is_train=True, test_categories=None):
        super().__init__(
            cfg, is_train=is_train, test_categories=test_categories
        )
    ...
    def __call__(self, dataset_dict):
        ...
        image_key, image_shape_key, transforms_key = self.transform_img(
            image_key
        )
        dataset_dict["image"].append(image_key)
        image_ref, image_shape_ref, transforms_ref = self.transform_img(
            image_ref
        )
        ...
        instances_key, expressions_new_key = self.transform_annos(
            annotations_key, transforms_key, image_shape_key, dataset_dict
        )
        instances_ref, expressions_new_ref = self.transform_annos(
            annotations_ref, transforms_ref, image_shape_ref, dataset_dict
        )
        # add "expressions" for detection data
        dataset_dict["expressions"].append(expressions_new_key)
        dataset_dict["expressions"].append(expressions_new_ref)

        instances_key_tmp = utils.filter_empty_instances(
            copy.deepcopy(instances_key)
        )
        instances_ref_tmp = utils.filter_empty_instances(
            copy.deepcopy(instances_ref)
        )
        ...
        dataset_dict["instances"] = [
            filter_empty_instances_soft(instances_key),
            filter_empty_instances_soft(instances_ref),
        ]  # instances of two frames
...
def filter_empty_instances_soft(
    instances, by_box=True, by_mask=True, box_threshold=1e-5
):
    """
...
def check_for_positive_overflow(
    instances, ind_to_class, tokenizer, max_seq_length=256
):
    ...
    for i in range(len(instances)):
        label_i = (
            instances.gt_classes[i].item() + 1
        )  # "+1" for mapping 0~79 to 1~80
...
def convert_object_detection_to_grounding_optimized_for_od(
    ...
    def generate_senetence_given_labels(
        ...
        if add_detection_prompt:  # False
            if (
                ...
                and (num_negatives == 0 or num_positives == 0)
                and not disable_shuffle
            ):
    ...
    if disable_shuffle:  # False
        ...
        full_positive = len(
            positive_label_set
        )  # num classes containing in the current image
        ...
        for negative_label in negative_label_list:
            label_text = (
                clean_name(ind_to_class[negative_label]) + ". "
            )  # "dog. "
    ...
    for i in range(len(instances)):
        ...
        label_i = instances.gt_classes[i].item() + 1
        if (
            label_i in label_to_positions
        ):  # NOTE: Only add those that actually appear in the final caption
...
def generate_control_options_given_probabilities(
    control_probabilities, full_positive, full_negative
):
    ...
    if outer_prob < probability_one_negative:
        ...
        outer_prob
        < probability_full + probability_one_positive + probability_one_negative
    ):  # prob 0.5
        ...
        if (
            random.random() < 1.0
        ):  # - probability_random_negative: probability of randomly sample X negatives (100%)
            num_negatives = (
                np.random.choice(max(1, full_negative)) + 1
            )  # mininum 1
        else:
...
class ConvertCocoPolysToMask(object):
    ...
    def __call__(self, target):
        ...
        if self.return_tokens and self.tokenizer is not None:  # True
            ...
            )  # (N, 256) N is num of objects. value > 0 where positive class
...
def create_positive_dict(tokenized, tokens_positive, labels):
    ...
    # tokens_positive: list of list of tuples. each tuple is (beg, end) of a token
