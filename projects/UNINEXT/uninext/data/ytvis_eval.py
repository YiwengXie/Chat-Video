...
class YTVISEvaluator(DatasetEvaluator):
    ...
    def reset(self):
        if self.dataset_name.startswith("bdd"):
            ...
            self.dataset_name.startswith("refytb-val")
            or self.dataset_name.startswith("rvos-refytb-val")
    ...
    def process(self, inputs, outputs):
        ...
        """
        if self.dataset_name.startswith("bdd"):
            prediction, _, _ = instances_to_coco_json_video_bdd(inputs, outputs)
    ...
    def evaluate(self):
        ...
        if self._distributed:
            ...
            if self.dataset_name.startswith("bdd"):
                if comm.is_main_process():
                    ...
                    for k in keys:
                        predictions_all[k] = list(
                            itertools.chain(*[p[k] for p in predictions])
                        )
        ...
        if self._output_dir:
            ...
            if self.dataset_name.startswith("bdd"):
                ...
                file_name = "instances_predictions_init_%.2f_obj_%.2f.pkl" % (
                    init_score,
                    obj_score,
                )
...
def instances_to_coco_json_video(inputs, outputs):
    ...
    video_height = inputs[0]["height"]
    video_width = inputs[0]["width"]
    file_names = inputs[0]["file_names"]
    data_source_name = file_names[0].split("/")[1]

    if data_source_name == "ytvis_2019":
        cate_map = YTVIS_CATEGORIES_2019
    elif data_source_name == "ytvis_2021":
        cate_map = YTVIS_CATEGORIES_2021
    elif data_source_name == "ovis":
        cate_map = OVIS_CATEGORIES
    elif data_source_name == "coco":
        cate_map = COCO_CATEGORIES

    scores = outputs["pred_scores"]
    labels = outputs["pred_labels"]
    masks = outputs.get("pred_masks", None)
    boxes = outputs["pred_bboxes"]
    frame_captions = outputs.get("frame_captions", None)

    H, W = inputs[0]["height"], inputs[0]["width"]
    dummy_arr = np.zeros((H, W), dtype=np.uint8)
    dummy_seg = mask_util.encode(
        np.array(dummy_arr[:, :, None], order="F", dtype="uint8")
    )[0]
    ytvis_results = []
    predictions = {}
    if masks is None:
    ...
    for instance_id, (s, l, m, b) in enumerate(
            ...
            for _mask in m:
                if _mask is None:
                    ...
                    segms.append(
                        mask_util.encode(
                            np.array(
                                _mask[:, :, None], order="F", dtype="uint8"
                            )
                        )[0]
                    )
        ...
        predictions[instance_id]["category"] = cate_map[l]["name"]
        predictions[instance_id]["boxes"] = b

    # video_info, chat = generate_caption(predictions, video_height, video_width, video_length / fps, fps, frame_captions)
    return ytvis_results, predictions, frame_captions
