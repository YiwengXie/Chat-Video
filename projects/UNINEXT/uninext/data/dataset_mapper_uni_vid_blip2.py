...
from .coco_dataset_mapper_uni import (
    AutoTokenizer,
...
openai.api_key = "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"

prompt_dict = {
    "sot": 'I am annotating a dense captioning dataset. Please help me. There is a moving object in a video, I will first tell you its coordinates in several frames of the video, please summarize the video for me. Note that the data format is "time, (coordinate); time, (coordinate); ...". The coordinate is described in the format: (x1, y1, x2, y2). Please include a brief description of the target object, and how its coordinates (x1, y1, x2, y2) change over time at the beginning of your introduction, and describe in detail the motion of the object. At the end of your description, please summarize the video for me. Do you understand?',
    "detection": 'I am annotating a dense captioning dataset. Please help me. There are multiple moving objects in a video, I will tell you their category names and coordinates in several frames of the video, please summarize the video for me. Note that the data format is "time: instance index, category name, (coordinate), instance index, category name, (coordinate), ...; time: instance index, category name, (coordinate), ...; ...". The coordinate is described in the format: (x1, y1, x2, y2). Please include a brief description of each object, and how its coordinates (x1, y1, x2, y2) change over time at the beginning of your introduction, and describe in detail the motion of different objects and their interaction with each other. At the end of your description, please summarize the video for me. Do you understand?',
    "grounding": 'I am annotating a dense captioning dataset. Please help me. There is a moving object in a video, I will describe its appearanace or location in the first frame, and tell you its coordinates in several frames of the video, please summarize the video for me. Note that the data format is "The description of the target object in the first frame. time, (coordinate); time, (coordinate); ...". The coordinate is described in the format: (x1, y1, x2, y2). Please include a brief description of the target object, and how its coordinates (x1, y1, x2, y2) change over time at the begining of your description, and describe in detail the motion of the object. At the end of your description, please summarize the video for me. Do you understand?',
}

__all__ = ["BLIP2_UniVidDatasetMapper"]


def generate_caption(
    dataset_dict, fps, selected_frames, category_map, role="user"
):
    ...
    # print("Reply1: ", reply_content + "\n")

    message_history.append({"role": "assistant", "content": f"{reply_content}"})

    instruction = (
        "Here is an example: \nThis is annotated for {} task.\n".format(task)
    )

    if task == "sot":
        boxes = instances[0].gt_boxes.tensor.numpy()
        instruction += "\n"
        for i, inst in enumerate(instances):
            frame_idx = selected_frames[i]
            box = inst.gt_boxes.tensor.numpy()
            instruction += "{}: ({}, {}, {}, {}); ".format(
                frame_idx / fps, box[0][0], box[0][1], boxes[0][2], box[0][3]
            )
        instruction += "\n"

    elif task == "detection":
        for i, inst in enumerate(instances):
            boxes = inst.gt_boxes.tensor.numpy()
            ins_ids = inst.gt_ids.numpy()
            cls_ids = inst.gt_classes.numpy()
            frame_idx = selected_frames[i]
            instruction += "\n{}: ".format(frame_idx / fps)
            for j, (ins_id, cls_id, box) in enumerate(
                zip(ins_ids, cls_ids, boxes)
            ):
                if ins_id == -1:
                    continue
                category_name = category_map[cls_id]["name"]
                instruction += "{}, {}, ({}, {}, {}, {}); ".format(
                    ins_id, category_name, box[0], box[1], box[2], box[3]
                )
            instruction += "\n"

    elif task == "grounding":
        expressions = dataset_dict["expressions"][0]
        instruction += "The target object is " + expressions + "\n"
        for i, inst in enumerate(instances):
            frame_idx = selected_frames[i]
            box = inst.gt_boxes.tensor.numpy()
            instruction += "{}: ({}, {}, {}, {}); ".format(
                frame_idx / fps, box[0][0], box[0][1], box[0][2], box[0][3]
            )
        instruction += "\n"

    instruction2 = (
        instruction
        + 'Your answer should be like: "at t seconds, it moves to (x1, y1, x2, y2)". If there are multiple target objects, please indicate the number of objects in each frame. I hope you could describe the motion of the target objects and their actions in detail. You may not see these objects, but you could infer their interactions based on their spatial locations and categories. Please start your answer with "In the given video" instead of "Sure".'
    )

    message_history.append({"role": "user", "content": instruction2})
    completion = openai.ChatCompletion.create(
        model="gpt-3.5-turbo", messages=message_history
    )
    ...
    print("Instruction: {} \n\nReply: {}\n".format(instruction, reply_content))
...
class BLIP2_UniVidDatasetMapper:
    """
        ...
        if self.lang_guide_det:
            ...
            for dataset_name in datasets:
                if dataset_name.startswith("ytvis_2019"):
                    self.ind_to_class_dict["vis19"] = cat2ind(
                        YTVIS_CATEGORIES_2019
                    )
                elif dataset_name.startswith("ytvis_2021"):
                    self.ind_to_class_dict["vis21"] = cat2ind(
                        YTVIS_CATEGORIES_2021
                    )
                    ...
                    self.ind_to_class_dict["bdd_det"] = cat2ind(
                        BDD_DET_CATEGORIES
                    )
                elif dataset_name.startswith("bdd_inst"):
                    self.ind_to_class_dict["bdd_inst"] = cat2ind(
                        BDD_INST_CATEGORIES
                    )
                elif dataset_name.startswith(
                    "bdd_box_track"
                ) or dataset_name.startswith("bdd_seg_track"):
                    self.ind_to_class_dict["bdd_track"] = cat2ind(
                        BDD_TRACK_CATEGORIES
                    )
                elif (
                    dataset_name.startswith("refytb-val")
                    or dataset_name.startswith("rvos")
                    or dataset_name.startswith("sot")
                ):
                    pass
                else:
                    raise ValueError(
                        "Unsupported dataset_name: %s" % dataset_name
                    )
            use_roberta = (
                cfg.MODEL.LANGUAGE_BACKBONE.TOKENIZER_TYPE == "roberta-base"
                and cfg.MODEL.LANGUAGE_BACKBONE.MODEL_TYPE == "roberta-base"
            )
            if use_roberta:
                self.tokenizer = RobertaTokenizerFast.from_pretrained(
                    "projects/UNINEXT/roberta-base"
                )
            else:
            ...
            if test_categories is not None:
                (
                    ...
                    test_categories, self.tokenizer
                ...
                for dataset_name in datasets:
                    if dataset_name.startswith("ytvis_2019"):
                        ...
                        (
                            ...
                            OVIS_CATEGORIES, self.tokenizer
                        )
                        ...
                        (
                            ...
                            COCO_CATEGORIES, self.tokenizer
                        )
                        ...
                        (
                            ...
                            BDD_DET_CATEGORIES, self.tokenizer
                        )
                        ...
                        (
                            ...
                            BDD_INST_CATEGORIES, self.tokenizer
                        )
    ...
    def from_config(cls, cfg, is_train: bool = True, test_categories=None):
        ...
        if cfg.DATALOADER.SAMPLER_TRAIN == "MultiDatasetSampler" and is_train:
            multidataset = True
            assert len(cfg.INPUT.MIN_SIZE_TRAIN_MULTI) == len(
                cfg.INPUT.MAX_SIZE_TRAIN_MULTI
            )
            augs_nocrop, augs = [], []
            for (min_size_train, max_size_train) in zip(
                cfg.INPUT.MIN_SIZE_TRAIN_MULTI, cfg.INPUT.MAX_SIZE_TRAIN_MULTI
            ):
                if cfg.INPUT.CROP.ENABLED and is_train:
                    augs_nocrop_cur, augs_cur = build_augmentation(
                        cfg, is_train, min_size_train, max_size_train
                    )
                else:
                    augs_cur = build_augmentation(
                        cfg, is_train, min_size_train, max_size_train
                    )
    ...
    def __call__(self, dataset_dict):
        ...
        if self.is_train:
            if video_length == 1:
                ...
                dataset_dict["file_names"] = (
                    dataset_dict["file_names"] * self.sampling_frame_num
                )
                dataset_dict["annotations"] = (
                    dataset_dict["annotations"] * self.sampling_frame_num
                )
        ...
        for frame_idx in selected_idx:
            ...
            if self.multidataset and self.is_train:
                transforms = selected_augmentations[
                    dataset_dict["dataset_source"]
                ](aug_input)
            else:
                transforms = selected_augmentations(aug_input)
            if task == "grounding":
                dataset_dict["expressions_ground"].append(
                    self.transform_expressions(
            ...
            if task in ["detection", "grounding"]:
                if (video_annos is None) or (not self.is_train):
                    if self.lang_guide_det and task == "detection":
                        dataset_dict["expressions"].append(
                            self.prompt_test_dict[dataset_dict["dataset_name"]]
                        )
                        if frame_idx == 0:
                            dataset_dict[
                                "positive_map_label_to_token"
                ...
                if not self.is_train:
                    ...
                    ori_id_list = [
                        x["ori_id"] if "ori_id" in x else None for x in annos
                    ]
            ...
            sorted_annos = [
                _get_dummy_anno(has_mask=has_mask) for _ in range(len(ids))
            ]
            ...
            if self.lang_guide_det and task == "detection":
                ind_to_class = self.ind_to_class_dict[
                    dataset_dict["dataset_name"]
                ]
                original_box_num = len(instances)
                (
                    instances,
                    positive_caption_length,
                ) = check_for_positive_overflow(
                    instances,
                    ind_to_class,
                    self.tokenizer,
                ...
                instances.positive_map = anno[
                    "positive_map"
                ].bool()  # (N, max_seq_len). N is num of objects. bool() -> 0 or 1
                expressions_new = anno[
                    "caption"
                ]  # "expressions" are shared between detection and grounding
                # postive_map must be consistent with expressions！
                dataset_dict["expressions"].append(
                    expressions_new
                )  # expression for the key frame
            elif self.lang_guide_det and task == "grounding":
                if len(instances) != 0:
                    instances.positive_map = torch.ones(
                        (1, 1), dtype=torch.bool
                    )  # 1 instance, 1 (pooled) token.
                else:
                    print(
            ...
            instances_tmp = utils.filter_empty_instances(
        ...
        if task == "sot":
            ...
            for instances in dataset_dict["instances"]:
                instance_ids = instances.gt_ids.tolist()
                instance_ids_valid = [x for x in instance_ids if x != -1]
                instance_ids_lists.append(instance_ids_valid)

            valid_ids_all = instance_ids_lists[0]
            for instance_ids in instance_ids_lists[1:]:
                valid_ids_all = list(set(valid_ids_all) & set(instance_ids))

            if len(valid_ids_all) == 0:
                return None
            else:
                pick_id = random.choice(valid_ids_all)
                """new_instances = []
        ...
        # print("dataset name: {}, selected_idx: {} ".format(file_names[0], selected_idx))
        data_source_name = file_names[0].split("/")[1]
        fps = fps_dicts[data_source_name]

        if data_source_name == "coco":
            category_map = COCO_CATEGORIES
        elif data_source_name == "bdd":
            if "seg_track" in file_names[0]:
                category_map = BDD_INST_CATEGORIES
            elif "track" in file_names[0]:
                category_map = BDD_TRACK_CATEGORIES
            else:
                category_map = BDD_DET_CATEGORIES
        elif data_source_name == "ytvis_2019":
            category_map = YTVIS_CATEGORIES_2019
        elif data_source_name == "ovis":
            category_map = OVIS_CATEGORIES
        else:
            category_map = None
        ...
        for _ in range(num_tries):
            try:
                video_summary = generate_caption(
                    dataset_dict, fps, selected_idx, category_map
                )
                break
            except:
                print("video summary generation failed, retrying...")
                continue

        dataset_dict["video_summary"] = video_summary"""
