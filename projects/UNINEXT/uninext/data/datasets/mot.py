...
def acc_single_video(
    results, gts, iou_thr=0.5, ignore_iof_thr=0.5, ignore_by_classes=False
):
    ...
    for result, gt in zip(results, gts):
        if ignore_by_classes:
            gt_ignore = bbox2result(
                gt["bboxes_ignore"], gt["labels_ignore"], num_classes
            )
        else:
            gt_ignore = [gt["bboxes_ignore"] for i in range(num_classes)]
        gt = track2result(
            gt["bboxes"], gt["labels"], gt["instance_ids"], num_classes
        )
        for i in range(num_classes):
            gt_ids, gt_bboxes = gt[i][:, 0].astype(np.int), gt[i][:, 1:]
            pred_ids, pred_bboxes = (
                result[i][:, 0].astype(np.int),
                result[i][:, 1:-1],
            )
