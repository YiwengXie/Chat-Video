...
def _get_ytvis_2021_instances_meta():
    ...
    thing_colors = [
        k["color"] for k in YTVIS_CATEGORIES_2021 if k["isthing"] == 1
    ]
    assert len(thing_ids) == 40, len(thing_ids)
    # Mapping from the incontiguous YTVIS category id to an id in [0, 39]
    thing_dataset_id_to_contiguous_id = {k: i for i, k in enumerate(thing_ids)}
    thing_classes = [
        k["name"] for k in YTVIS_CATEGORIES_2021 if k["isthing"] == 1
    ]
    ret = {
        "thing_dataset_id_to_contiguous_id": thing_dataset_id_to_contiguous_id,
        "thing_classes": thing_classes,
...
"""
    ...
    logger.info(
        "Loaded {} videos in YTVIS format from {}".format(
            len(vids_anns), json_file
        )
    )
...
if __name__ == "__main__":
    ...
    dicts = load_ytvis_json(
        json_file, image_root, dataset_name="ytvis_2019_train"
    )
