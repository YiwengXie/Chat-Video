...
class DDETRSegmUniDN(nn.Module):
    def __init__(
        ...
        for l in range(self.controller_layers):
            if l == 0:
                if self.rel_coord:
                    ...
                    weight_nums.append(
                        self.in_channels * self.dynamic_mask_channels
                    )
        ...
        if self.dynamic_label_enc:
            ...
            self.num_classes = (
                80  # TODO: This is a hard-code, which needs to be corrected.
            )
    ...
    def coco_forward(
        ...
        for lvl in range(enc_lay_num):
            ...
            if task == "grounding":
                outputs_class = self.detr.class_embed[lvl](
                    hs[lvl], lang_feat_pool
                )
            elif task == "detection":
                outputs_class = self.detr.class_embed[lvl](
                    hs[lvl], language_dict_features["hidden"]
                )
            else:
                raise ValueError("task must be detection or grounding")
        ...
        if task == "grounding":
            bs, device = (
                language_dict_features["masks"].size(0),
                language_dict_features["masks"].device,
            )
    ...
    def coco_inference(
        ...
        if hasattr(self, "reid_embed_head"):
            if self.use_deformable_reid_head:
                inst_embed = self.reid_embed_head[1](
                    self.reid_embed_head[0](
                        hs[-1],
                        src_info["reference_points"],
                        src_info["src"],
                        src_info["src_spatial_shapes"],
                        src_info["src_level_start_index"],
                        src_info["src_valid_ratios"],
                        src_info["src_padding_mask"],
                    )
                )
            else:
        ...
        if not train and pred_mask:
            ...
            for i, image_size_i in enumerate(image_sizes):
                ...
                orig_h = torch.as_tensor(orig_h).to(
                    outputs["reference_points"][i]
                )
                orig_w = torch.as_tensor(orig_w).to(
                    outputs["reference_points"][i]
                )
                scale_f = torch.stack([orig_w, orig_h], dim=0)
    ...
    def prepare_for_cdn(
        ...
        known = [
            (torch.ones_like(t["labels"])).cuda() for t in targets
        ]  # [[1]*num_tgts_sam0, [1]*num_tgts_sam1, ...]
        batch_size = len(known)
        known_num = [
            sum(k) for k in known
        ]  # [num_tgts_sam0, num_tgts_sam1, ...]
        if int(max(known_num)) == 0:
            return None, None, None, None

        dn_number = dn_number // (
            int(max(known_num) * 2)
        )  # num of dn-group, shared in a batch

        if dn_number == 0:
            dn_number = 1
        unmask_bbox = unmask_label = torch.cat(
            known
        )  # [ [1]*num_tgts_sam0 + [1]*num_tgts_sam1 + ... ]
        labels = torch.cat(
            [t["labels"] for t in targets]
        )  # [label0, label1, ...]
        boxes = torch.cat([t["boxes"] for t in targets])  # [box0, box1, ...]
        ...
        )  # [[0]*num_tgts_sam0, [1]*num_tgts_sam1, ... ]

        known_indice = torch.nonzero(unmask_label + unmask_bbox)
        known_indice = known_indice.view(
            -1
        )  # [0, 1, 2, 3, 4, 5, ..., num_tgts_sam0+num_tgts_sam1+...]
    ...
    def dn_post_process(
        self, outputs_class, outputs_coord, dn_metas, outputs_iou=None
    ):
        if dn_metas and dn_metas["single_padding"] > 0:
            ...
            if self.aux_loss:
                out["aux_outputs"] = self._set_aux_loss_dn(
    ...
    def dynamic_mask_with_coords(
        ...
        if rel_coord:
            ...
            relative_coords = relative_coords.permute(0, 1, 4, 2, 3).flatten(
                -2, -1
            )
            ...
            for i, num_inst in enumerate(num_insts):
                ...
                mask_head_b = torch.cat(
                    [relative_coords_b, mask_feats_b], dim=2
                )
        ...
        if self.use_raft:
            ...
            for b, n in enumerate(num_insts):
                mask_logits_output.append(
                    self.upsample_preds(
                        mask_logits[inst_idx : inst_idx + n],
                        up_masks[b : b + 1],
                    )
                )
                inst_idx += n
    ...
    def debug_data(self, samples, gt_targets):
        ...
        for i in range(len(gt_targets)):
            ...
            cv2.imwrite(
                "rank_%02d_batch_%d_img.jpg" % (dist.get_rank(), i), image
            )
            cv2.imwrite(
    ...
    def __init__(self, dim, fpn_dims, context_dim, use_raft=False, up_rate=4):
        ...
        if self.use_raft:
            self.up_mask_layer = nn.Sequential(
                ...
                nn.Conv2d(
                    context_dim, self.up_rate * self.up_rate * 9, 1, padding=0
                ),
            )
    ...
    def __init__(
        ...
        if self.use_raft:
            self.up_mask_layer = nn.Sequential(
                ...
                nn.ReLU(inplace=True),
                nn.Conv2d(
                    channels, self.up_rate * self.up_rate * 9, 1, padding=0
                ),
            )
...
class MHAttentionMap(nn.Module):
    ...
    def forward(self, q, k, mask=None):
        ...
        weights = torch.einsum(
            "bqnc,bnchw->bqnhw", qh * self.normalize_fact, kh
        )
