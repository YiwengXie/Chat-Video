...
class IDOL_Tracker(object):
        ...
        for k, v in self.tracklets.items():
            ...
            if self.long_match:
                ...
                if self.temporal_weight:
                    length = len(weights)
                    temporal_weight = torch.range(0.0, 1, 1 / length)[1:].to(
                        weights
                    )
                    weights = weights + temporal_weight
                sum_embed = (
                    torch.stack(v["long_embed"]) * weights.unsqueeze(1)
                ).sum(0) / weights.sum()
    ...
    def match(self, bboxes, labels, masks, track_feats, frame_id, indices):
        ...
        if bboxes.size(0) > 0 and not self.empty:
            ...
            for i in range(bboxes.size(0)):
                ...
                if self.frame_weight:
                    ...
                    if (scores[i, non_backs] > 0.5).sum() > 1:
                        ...
                        wighted_scores[i, ~non_backs] = (
                            wighted_scores[i, ~non_backs] * frame_weight.mean()
                        )
            ...
            new_inds = (ids == -2) & (
                bboxes[:, 4] > self.addnew_score_thr
            ).cpu()
            ...
            unselected_inds = torch.nonzero(ids == -2, as_tuple=False).squeeze(
                1
            )
