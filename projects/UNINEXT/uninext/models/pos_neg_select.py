...
def select_pos_neg(
    ...
    if use_deformable_reid_head:
        ...
        assert detach_reid
        ref_embeds = embed_head[1](
            embed_head[0](
                hs_ref.detach(),
                src_info_ref["reference_points"],
                src_info_ref["src"],
                src_info_ref["src_spatial_shapes"],
                src_info_ref["src_level_start_index"],
                src_info_ref["src_valid_ratios"],
                src_info_ref["src_padding_mask"],
            )
        )
        key_embedds = embed_head[1](
            embed_head[0](
                hs_key.detach(),
                src_info_key["reference_points"],
                src_info_key["src"],
                src_info_key["src_spatial_shapes"],
                src_info_key["src_level_start_index"],
                src_info_key["src_valid_ratios"],
                src_info_key["src_padding_mask"],
            )
        )
    else:
        if detach_reid:
    ...
    for bz_i, (v, detv, indices) in enumerate(
        zip(targets, det_targets, all_indices)
    ):
        ...
        contrastive_pos = get_pos_idx(
            ref_box_bz, ref_cls_bz, tgt_bbox, tgt_labels, tgt_valid
        )

        # loop over gts in one image
        for inst_i, (valid, matched_query_id) in enumerate(
            zip(tgt_valid, indices)
        ):
            ...
            neg_embed = ref_embeds[bz_i][
                ~contrastive_pos[1][inst_i]
            ]  # remove querys which may have overlap with the given gt. The rest querys are negative embed
            contrastive_embed = torch.cat([pos_embed, neg_embed], dim=0)
            contrastive_label = torch.cat(
                [one.repeat(len(pos_embed)), zero.repeat(len(neg_embed))], dim=0
            )

            contrast = torch.einsum(
                "nc,kc->nk", [contrastive_embed, key_embed_i]
            )  # (N_pos+N_neg, N_gt)
            ...
            if len(pos_embed) == 0:
                ...
                num_sample_neg = (
                    len(pos_embed) * 10
                )  # control num_neg (max is 10 * num_pos)
            ...
            aux_contrastive_label = torch.cat(
                [one.repeat(len(pos_embed)), zero.repeat(num_sample_neg)], dim=0
            )
            ...
            cosine = torch.einsum(
                "nc,kc->nk", [aux_contrastive_embed, key_embed_i]
            )
...
def get_pos_idx(bz_boxes, bz_out_prob, bz_gtboxs, bz_tgt_ids, valid):
    with torch.no_grad():
        ...
        fg_mask, is_in_boxes_and_center = get_in_boxes_info(
            bz_boxes, bz_gtboxs, expanded_strides=32
        )
        pair_wise_ious = ops.box_iou(
            box_cxcywh_to_xyxy(bz_boxes), box_cxcywh_to_xyxy(bz_gtboxs)
        )
        ...
        pos_cost_class = (
            alpha * ((1 - bz_out_prob) ** gamma) * (-(bz_out_prob + 1e-8).log())
        )
        cost_class = torch.zeros(
            (bz_out_prob.size(0), bz_tgt_ids.size(0)),
            device=pos_cost_class.device,
        )
        for idx in range(bz_tgt_ids.size(0)):
            cost_class[:, idx] = (
                pos_cost_class[:, bz_tgt_ids[idx]]
                - neg_cost_class[:, bz_tgt_ids[idx]]
            ).mean(
                -1
            )  # mean(-1) is to deal with situations where one class name is divided into multiple tokens.
        ...
        if False in valid:
            ...
            indices_batchi_neg = []
            if valid.sum() > 0:
                indices_batchi_pos_s = dynamic_k_matching(
                    cost, pair_wise_ious, int(valid.sum()), 10
                )
                indices_batchi_neg_s = dynamic_k_matching(
                    cost, pair_wise_ious, int(valid.sum()), 100
                )
            valid_idx = 0
            ...
            if valid.sum() > 0:
                indices_batchi_pos = dynamic_k_matching(
                    cost, pair_wise_ious, bz_gtboxs.shape[0], 10
                )
                indices_batchi_neg = dynamic_k_matching(
                    cost, pair_wise_ious, bz_gtboxs.shape[0], 100
                )
            else:
...
def get_in_boxes_info(boxes, target_gts, expanded_strides):
    ...
    center_radius = 2.5
    b_l = anchor_center_x > (
        target_gts[:, 0] - (1 * center_radius / expanded_strides)
    ).unsqueeze(0)
    b_r = anchor_center_x < (
        target_gts[:, 0] + (1 * center_radius / expanded_strides)
    ).unsqueeze(0)
    b_t = anchor_center_y > (
        target_gts[:, 1] - (1 * center_radius / expanded_strides)
    ).unsqueeze(0)
    b_b = anchor_center_y < (
        target_gts[:, 1] + (1 * center_radius / expanded_strides)
    ).unsqueeze(0)
