...
from .vlfusion import BertEncoderLayer, VLFuse
...
class DeformableTransformerVLDINO(nn.Module):
    def __init__(
        ...
        vl_fusion_layer = (
            VLFuse(cfg) if cfg.MODEL.USE_EARLY_FUSION else nn.Identity()
        )
        if cfg.MODEL.USE_ADDITIONAL_BERT:
            lang_cfg = BertConfig.from_pretrained(
                "projects/DDETRS/bert-base-uncased"
            )
            lang_encoder_layer = BertEncoderLayer(
                lang_cfg,
        ...
        self.resizer = FeatureResizer(
            input_feat_size=768, output_feat_size=d_model, dropout=0.1  # 256
        )
        # decoupled designs
    ...
    def forward(
        ...
        # { "masks" : bs x 256,
        #  "hidden" : bs x 256 x 768}

        # encoder & VL early fusion
        vl_feats_dict = self.encoder(
            src_flatten,
            spatial_shapes,
            level_start_index,
            valid_ratios,
        ...
        memory, language_dict_features = (
            vl_feats_dict["visual"],
            vl_feats_dict["lang"],
        )
        ...
        bs, _, c = memory.shape  # (bs, sigma(HW), 256)
        assert self.two_stage
        output_memory, output_proposals = self.gen_encoder_output_proposals(
            memory, mask_flatten, spatial_shapes
        )
        """
        memory torch.Size([2, sigma(HW), 256])
        output_memory torch.Size([2, sigma(HW), 256])
        output_proposals torch.Size([2, sigma(HW), 4])
        """

        # hack implementation for two-stage Deformable DETR
        enc_outputs_class = self.decoder.class_embed[self.decoder.num_layers](
            output_memory, lang_feat_pool.unsqueeze(1)
        )
            ...
            if self.decouple_tgt:
                if self.still_tgt_for_both:
                    ...
                    if task == "detection":
                        tgt_new = (
                            tgt + 0.0 * ref_feat
                        )  # "+ 0.0 *" is to avoid unused parameters
                    elif task == "grounding":
                        tgt_new = (
                            ref_feat + 0.0 * tgt
                        )  # "+ 0.0 *" is to avoid unused parameters
                        ...
                        raise ValueError(
                            "task should be detection or grounding"
                        )
            else:
                if query_embed[0] is None:
                    tgt_new = ref_feat.repeat(
                        1, self.two_stage_num_proposals, 1
                    )
                else:
                    tgt_new = torch.cat(
                        [
                            query_embed[0],
                            ref_feat.repeat(1, self.two_stage_num_proposals, 1),
                        ],
                        1,
                    )
...
class DeformableTransformerEncoderVL(nn.Module):
    def __init__(
        ...
        self.vl_layers = _get_clones_advanced(
            vl_fusion_layer, num_layers, num_vl_layers
        )
    ...
    def forward(
        ...
        for _, (vl_layer, layer, lang_layer) in enumerate(
            zip(self.vl_layers, self.layers, self.lang_layers)
        ):
    ...
    def forward(
        ...
        for lid, layer in enumerate(self.layers):
            ...
            if self.bbox_embed is not None:
                ...
                )  # new reference points in the next decoder layer
