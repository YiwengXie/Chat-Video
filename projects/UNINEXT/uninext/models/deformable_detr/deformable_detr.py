...
class VL_Align(nn.Module):
    def __init__(self, cfg):
        ...
        self.dot_product_projection_text = nn.Linear(
            cfg.MODEL.LANGUAGE_BACKBONE.LANG_DIM,
            cfg.MODEL.DDETRS.HIDDEN_DIM,
            bias=True,
        )  # 768 -> 256
        self.log_scale = nn.Parameter(
            torch.Tensor([cfg.MODEL.DYHEAD.LOG_SCALE]), requires_grad=True
        )
        self.bias_lang = nn.Parameter(
            torch.zeros(cfg.MODEL.LANGUAGE_BACKBONE.LANG_DIM),
            requires_grad=True,
        )  # (768，)
    ...
    def forward(self, x, embedding):
        ...
        embedding = F.normalize(
            embedding, p=2, dim=-1
        )  # (bs, L, 768) L is maximum sentence length
    ...
    def __init__(
        ...
        if with_box_refine:
            ...
            if cfg.MODEL.USE_IOU_BRANCH:
                self.iou_head = nn.ModuleList(
                    [self.iou_head for _ in range(num_pred - 1)]
                )
        ...
        if two_stage:
            ...
            if cfg.MODEL.STILL_CLS_FOR_ENCODER:
                prior_prob = 0.01
                bias_value = -math.log((1 - prior_prob) / prior_prob)
                self.transformer.decoder.class_embed[-1] = Still_Classifier(
                    hidden_dim
                )  # binary classification
                self.transformer.decoder.class_embed[-1].body.bias.data = (
...
class SetCriterion(nn.Module):
    ...
    def loss_labelsVL(self, outputs, targets, indices, num_boxes, log=False):
        ...
        if num_boxes == 0:
            ...
            positive_map = [
                x["positive_map"] for x in targets
            ]  # [(N1, 256), (N2, 256)]
            ...
            for batch_idx, (src_idxs, target_idxs) in enumerate(indices):
                ...
                for (src_idx, target_idx) in zip(src_idxs, target_idxs):
                    target_classes_onehot[batch_idx, src_idx] = positive_map[
                        batch_idx
                    ][target_idx]
            loss_ce = (
                token_sigmoid_binary_focal_loss(
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes):
        ...
        if "pred_boxious" in outputs:
            ...
            loss_boxiou = F.binary_cross_entropy_with_logits(
    ...
    def loss_masks(self, outputs, targets, indices, num_boxes):
        ...
        src_masks = outputs[
            "pred_masks"
        ]  # list[tensor]: bs x [1, num_inst, num_frames, H/4, W/4]
        bs = len(targets)
        ...
        if type(src_masks) == list:
            src_masks = torch.cat(src_masks, dim=1)[
                0
            ]  # [num_all_inst, num_frames, H/4, W/4]
        if src_masks.ndim == 0:
        ...
        target_masks = target_masks.reshape(
            bs, -1, num_frames, target_masks.shape[-2], target_masks.shape[-1]
        )
    ...
    def loss_masks_boxinst(self, outputs, targets, indices, num_boxes):
        ...
        if len(tgt_idx[0]) > self.boxinst_topk:
            keep_indexs = random.sample(
                range(len(tgt_idx[0])), self.boxinst_topk
            )
        ...
        for (batch_idx, target_idx) in zip(tgt_idx[0], tgt_idx[1]):
            image_color_similarity_list.append(
                targets[batch_idx]["image_color_similarity"][target_idx]
            )
        image_color_similarity = torch.stack(
            image_color_similarity_list, dim=0
        ).to(
        ...
        pairwise_losses = compute_pairwise_term(
            src_masks, self.pairwise_size, self.pairwise_dilation
        )

        weights = (
            image_color_similarity >= self.pairwise_color_thresh
        ).float() * target_masks.float()  # (N, 8, H//4, W//4)
    ...
    def loss_reid(self, outputs, targets, indices, num_boxes):
        ...
        if len(qd_items) == 0:
            losses = {
                "loss_reid": outputs["reid_params"] * 0,
        ...
        for qd_item in qd_items:
            ...
            _pos_expand = torch.repeat_interleave(
                pred_pos, pred.shape[1], dim=1
            )
        ...
        losses = {
            "loss_reid": contras_loss.sum() / len(qd_items),
            ...
            for bt in bin_targets:
                ...
                if self.still_cls_for_encoder:
                    if "positive_map" in bt:
                        ...
                        bt["positive_map"] = torch.ones(
                            (num_targets, 1),
                            dtype=torch.bool,
                            device=bt["positive_map"].device,
                        )
    ...
    def get_target_masks(self, targets, src_masks):
        target_masks, valid = nested_tensor_from_tensor_list(
            [t["masks"] for t in targets], size_divisibility=32, split=False
        ).decompose()
    ...
    def forward(self, outputs, targets, indices_list, dn_metas=None):
        ...
        Parameters:
             ...
             targets: list of dicts, such that len(targets) == batch_size.
                      The expected keys in each dict depends on the losses applied, see each loss' doc
        """
        losses = super(DINOCriterion, self).forward(
            outputs, targets, indices_list
        )
