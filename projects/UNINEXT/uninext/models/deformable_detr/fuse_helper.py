...
class BiMultiHeadAttention(nn.Module):
    def __init__(
        ...
        self.stable_softmax_2d = (
            cfg.MODEL.DYHEAD.FUSE_CONFIG.STABLE_SOFTMAX_2D
        )  # False
    ...
    def forward(self, v, l, attention_mask_l=None):
        ...
        query_states = self._shape(query_states, tgt_len, bsz).view(
            *proj_shape
        )  # (bs * 8, seq_len_img, embed_dim//8)
        key_states = key_states.view(
            *proj_shape
        )  # (bs * 8, seq_len_text, embed_dim//8)
