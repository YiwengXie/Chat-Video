...
class HungarianMatcher(nn.Module):
    ...
    def forward_ota(self, outputs, targets, nf=1):
        ...
        with torch.no_grad():
            ...
            for batch_idx in range(bs):
                cost, pair_wise_ious, bz_gtboxs = self.compute_cost(
                    batch_idx, out_bbox, out_prob, targets, nf
                )
                if bz_gtboxs.shape[0] > 0:
                    ...
                    indices_i = torch.tensor([], dtype=torch.int64).to(
                        out_prob.device
                    )
                    indices_j = torch.tensor([], dtype=torch.int64).to(
                        out_prob.device
                    )
    ...
    def compute_cost(self, batch_idx, out_bbox, out_prob, targets, nf):
        ...
        bz_gtboxs = targets[batch_idx]["boxes"].reshape(num_insts, nf, 4)[
            :, 0
        ]  # [num_gt, 4]
        # import pdb;pdb.set_trace()
    ...
    def get_in_boxes_info(self, boxes, target_gts, expanded_strides):
        ...
        is_in_boxes_anchor = (
            is_in_boxes_all | is_in_centers_all
        )  # 上述两个条件只要满足一个，就成为候选区域。注意！！！这里是“|”求或
    ...
    def dynamic_k_matching(self, cost, pair_wise_ious, num_gt):
        matching_matrix = torch.zeros_like(
            cost
        )  # [300,num_gt] matching_matrix表示映关系，必须保证只能1gt对多query，且每个gt 有query
...
class HungarianMatcherVL(nn.Module):
    ...
    def compute_cost(self, batch_idx, out_bbox, out_prob, targets, nf):
        bz_boxes = out_bbox[batch_idx]  # [300,4]
        bz_out_prob = out_prob[batch_idx]
        bz_tgt_ids = targets[batch_idx][
            "positive_map"
        ]  # (N_obj, 256) or (N_obj, 1)
        num_insts = len(bz_tgt_ids)
        bz_gtboxs = targets[batch_idx]["boxes"].reshape(num_insts, nf, 4)[
            :, 0
        ]  # [num_gt, 4]
        ...
        cost_class = torch.zeros(
            (bz_out_prob.size(0), bz_tgt_ids.size(0)), device=out_prob.device
        )
    ...
    def forward(self, outputs, targets):
        ...
        with torch.no_grad():
            ...
            )  # (bs * num_queries, 256). 256 is max_seq_len
            out_bbox = outputs["pred_boxes"].flatten(
                0, 1
            )  # [batch_size * num_queries, 4]
            ...
            tgt_ids = torch.cat(
                [v["positive_map"] for v in targets]
            )  # (N_obj, 256) or (N_obj, 1)
            tgt_bbox = torch.cat([v["boxes"] for v in targets])
            ...
            cost_class = torch.zeros(
                (out_prob.size(0), tgt_ids.size(0)), device=out_prob.device
            )
            for idx in range(tgt_ids.size(0)):
                cost_class[:, idx] = (
                    pos_cost_class[:, tgt_ids[idx]]
                    - neg_cost_class[:, tgt_ids[idx]]
                ).mean(
                    -1
                )  # mean(-1) is to deal with situations where one class name is divided into multiple tokens.
            ...
            indices = [
                linear_sum_assignment(c[i])
                for i, c in enumerate(C.split(sizes, -1))
            ]  # List of length batchsize. Each element is a tuple of two arrays.
            return [
                (
                    torch.as_tensor(i, dtype=torch.int64),
                    torch.as_tensor(j, dtype=torch.int64),
                ...
                for i, j in indices
            ]  # List of length batchsize. Each element is a tuple of two tensors.
