...
class DDETRSegmUniVIDDN(DDETRSegmUniDN):
    def __init__(
        ...
        mask_out_stride=4,
        template_sz=256,
        extra_backbone_for_template=False,
        search_area_factor=2,
        ref_feat_sz=8,
        sot_feat_fusion=False,
        ...
        super().__init__(
            ...
            rel_coord=rel_coord,
            ota=ota,
            new_mask_head=new_mask_head,
            use_raft=use_raft,
            mask_out_stride=mask_out_stride,
            use_iou_branch=use_iou_branch,
            decouple_tgt=decouple_tgt,
            cfg=cfg,
        )
        ...
        if cfg.USE_DEFORMABLE_REID_HEAD:
            ...
            self.reid_embed_head = nn.ModuleList(
                [
                    DeformableReidHead(
                        hidden_dim, decoder_layer, cfg.N_LAYER_DEFORMABLE_REID
                    ),
    ...
    def get_template(self, img, mask, bbox):
        ...
        # Crop target
        im_crop = img[
            :, :, y1 + y1_pad : y2 - y2_pad, x1 + x1_pad : x2 - x2_pad
        ]
        mask_crop = mask[
            :, :, y1 + y1_pad : y2 - y2_pad, x1 + x1_pad : x2 - x2_pad
        ]
        ...
        im_crop_padded = F.interpolate(
            im_crop_padded,
            (self.template_sz, self.template_sz),
            mode="bilinear",
            align_corners=False,
        )
    ...
    def get_template_4c(self, img, pad_mask, bbox, gt_mask=None):
        ...
        if gt_mask is None:
            ...
            mask_crop_padded_gt[
                :, :, max(0, y1_t) : y2_t, max(0, x1_t) : x2_t
            ] = 1
        ...
        mask_crop_padded_gt = F.interpolate(
            mask_crop_padded_gt,
            (self.template_sz, self.template_sz),
            mode="bilinear",
            align_corners=False,
        )
        im_crop_padded_4c = torch.cat(
            [im_crop_padded, mask_crop_padded_gt], dim=1
        )
    ...
    def debug_template(self, samples):
        ...
        for i in range(len(samples.tensors)):
            ...
            cv2.imwrite(
                "rank_%02d_batch_%d_template_img.jpg" % (dist.get_rank(), i),
                image,
            )
            cv2.imwrite(
                "rank_%02d_batch_%d_template_mask.jpg" % (dist.get_rank(), i),
                input_mask,
            )
        import sys
    ...
    def debug_template_4c(self, samples):
        ...
        for i in range(len(samples.tensors)):
            image_mask = (
                samples.tensors[i].permute((1, 2, 0)).cpu().numpy()
            )  # (H, W, 4)
            image = image_mask[:, :, :3]
    ...
    def forward_backbone(self, samples, template_branch=False):
        ...
        if self.detr.num_feature_levels > len(features):
            ...
            for l in range(_len_srcs, self.detr.num_feature_levels):
                ...
                pos_l = self.detr.backbone[1](NestedTensor(src, mask)).to(
                    src.dtype
                )
                srcs.append(src)
                masks.append(mask)
    ...
    def coco_forward_sot(
        ...
        samples_key = NestedTensor(
            samples.tensors[key_ids], samples.mask[key_ids]
        )
        srcs_key, masks_key, poses_key, spatial_shapes = self.forward_backbone(
            samples_key
        )
        ...
        for batch_idx in range(bz):
            ...
            if self.extra_backbone_for_template:
                template_tensor, template_mask = self.get_template_4c(
                    img, mask, bbox, gt_mask=ref_gt_mask
                )
            else:
                template_tensor, template_mask = self.get_template(
                    img, mask, bbox
                )

            ref_tensor_list.append(template_tensor)
            ref_mask_list.append(template_mask)
        ref_tensor = torch.cat(ref_tensor_list, dim=0)  # bs, 4, 256, 256
        ref_mask = torch.cat(ref_mask_list, dim=0)  # bs, 256, 256
        samples_ref = NestedTensor(ref_tensor, ref_mask)

        srcs_ref, masks_ref, _, _ = self.forward_backbone(
            samples_ref, template_branch=True
        )  # (bs, 256, 32, 32), (bs, 32, 32)

        language_dict_features_key = {}
        if self.sot_feat_fusion:
            ref_feats = (
                self.sot_fuser(srcs_ref).flatten(-2).permute(0, 2, 1)
            )  # (bs, 1024, 256)
            ref_masks = masks_ref[0].flatten(-2)  # (bs, 1024)

        else:
            ref_feats, ref_masks = [], []
            for n_l in range(self.detr.num_feature_levels):
                ref_feat_l = F.interpolate(
                    srcs_ref[n_l], size=(self.ref_feat_sz, self.ref_feat_sz)
                )  # (bs, C, 8, 8)
                ref_feats.append(ref_feat_l.flatten(-2))  # (bs, C, 64)
                ref_mask_l = F.interpolate(
                    masks_ref[n_l][None].float(),
                    size=(self.ref_feat_sz, self.ref_feat_sz),
                )[
                    0
                ].bool()  # (bs, 8, 8)
                ref_masks.append(ref_mask_l.flatten(-2))  # (bs, 64)
            ref_feats = torch.cat(ref_feats, dim=-1).permute(
                0, 2, 1
            )  # (bs, C, L) L=256 --> (bs, L, C)
            ref_masks = torch.cat(ref_masks, dim=-1)  # (bs, L)
        # original masks need to be reversed as text_masks!
        (
            language_dict_features_key["hidden"],
            language_dict_features_key["masks"],
        ) = (self.adjust_layer(ref_feats), ~ref_masks)
        ...
        (
            ...
            query_embeds,
            mask_on=True,
            language_dict_features=language_dict_features_key,
            task=task,
            attn_masks=attn_mask,
        )

        if task == "grounding" or task == "sot":
            lang_feat_pool_key = agg_lang_feat(
                language_dict_features_key["hidden"],
                language_dict_features_key["masks"],
            ).unsqueeze(
                1
        ...
        for lvl in range(enc_lay_num):
            ...
            if task == "grounding" or task == "sot":
                outputs_class = self.detr.class_embed[lvl](
                    hs[lvl], lang_feat_pool_key
                )
            elif task == "detection":
                outputs_class = self.detr.class_embed[lvl](
                    hs[lvl], language_dict_features_key["hidden"]
                )
            else:
            ...
            if self.ota:
                indices, matched_ids = criterion.matcher.forward_ota(
                    outputs_layer, det_targets
                )
            else:
        ...
        if task == "grounding" or "sot":
            bs, device = (
                language_dict_features_key["masks"].size(0),
                language_dict_features_key["masks"].device,
            )
        ...
        if self.detr.aux_loss:
            if self.use_iou_branch:
                outputs["aux_outputs"] = self._set_aux_loss_with_iou(
                    outputs_class, outputs_coord, outputs_mask, outputs_iou
                )
            else:
    ...
    def coco_inference_ref(self, samples, gt_targets):
        ...
        for batch_idx in range(bz):
            ...
            if "masks" in gt_targets[batch_idx]:
                ref_gt_mask = gt_targets[batch_idx]["masks"][
                    None
                ]  # (1, 1, H, W)
            else:
                ref_gt_mask = None
    ...
    def coco_forward_vis(
        ...
        query_embeds = (
            input_query_label,
            input_query_bbox,
        )  # bz x num_queries x 256, bz x num_queries x 4
        ...
        if self.use_deformable_reid_head:
            ...
            (
                ...
                [None, None],
                mask_on=True,
                language_dict_features=language_dict_features_ref,
                task=task,
                return_src_info=True,
            )
            if dn_meta and dn_meta["single_padding"] > 0:
                padding_size = dn_meta["single_padding"] * dn_meta["dn_num"]
                src_info_key["reference_points"] = inter_references[-1][
                    :, padding_size:, :
                ].detach()
            else:
        ...
        if task == "grounding":
            ...
            lang_feat_pool_ref = agg_lang_feat(
                language_dict_features_ref["hidden"],
                language_dict_features_ref["masks"],
            ).unsqueeze(
                1
            )  # (bs, 1, 768)
        elif task == "detection":
        ...
        for lvl in range(enc_lay_num):
            ...
            if self.ota:
                indices, matched_ids = criterion.matcher.forward_ota(
                    outputs_layer, det_targets
                )  # len(indices) = bz * num_det, len(matched_ids) = bz * num_det
            else:
                indices = criterion.matcher.forward(outputs_layer, det_targets)
            indices_list.append(indices)
            reference_points, mask_head_params, num_insts = [], [], []
        ...
        if self.use_iou_branch:
            ...
            outputs_iou = None
        if task == "grounding":
            ref_cls = self.detr.class_embed[enc_lay_num - 1](
                hs_ref[-1], lang_feat_pool_ref
            ).sigmoid()
        else:
            ref_cls = self.detr.class_embed[enc_lay_num - 1](
                hs_ref[-1], language_dict_features_ref["hidden"]
            ).sigmoid()  # NOTE: self.detr.class_embed[-1] may cause conflict with two-stage !

        # Only compute Embedding Loss on the last layer of decoder
        contrast_items = select_pos_neg(
            inter_references_ref[-1],
            matched_ids,
            ref_targets,
            det_targets,
            self.reid_embed_head,
            hs[-1][:, padding_size:],
            hs_ref[-1],
            ref_cls,
            detach_reid=self.detach_reid,
            use_deformable_reid_head=self.use_deformable_reid_head,
            src_info_key=src_info_key,
