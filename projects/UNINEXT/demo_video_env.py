...
class VisualizationEnvDemo(object):
    def __init__(self, max_frames=100):
        ...
        self.predictor = OmniVLPredictor()
...
class OmniVLPredictor:
    ...
    def dynamic_des(self, track_rois, category):
        ...
        for i in range(num_segments):
            ...
            question = "Describe the environment of the image:"
            video_attributes = (
                self.captioner.ask(self.get_roi(track_rois[0][1]), question)
                .replace("\n", "")
                .lower()
            )
            appearances.append(video_attributes)

        if category == "environment":
            object_dynamics = self.dynamic_captioner.predictor(
                segments, customized_prompt=f"In the video, "
            )
        else:
            object_dynamics = self.dynamic_captioner.predictor(
                segments, customized_prompt=f"In the video, the {category} is "
            )
        ...
        for time_inter, app, dyn in zip(
            ...
            object_app_desp += f"At {start_time} seconds, {app}; "
