...
from projects.Omnivl.models.blip import (
...
class BLIP_VQA(nn.Module):
    ...
    def forward(
        ...
        if train:
            ...
            # print(answer.input_ids.size(), question_states.size())
            ...
            if inference == "generate":
                num_beams = 1
                question_states = (
                    question_output.last_hidden_state.repeat_interleave(
                        num_beams, dim=0
                    )
                )
                ...
                # print(bos_ids.shape, question_states.shape)
...
def blip_vqa(config, pretrained="", **kwargs):
    model = BLIP_VQA(
