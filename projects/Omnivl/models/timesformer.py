...
class Block(nn.Module):
    ...
    def forward(self, x, B, T, W):
        ...
        if self.attention_type in ["space_only", "joint_space_time"] or T == 1:
...
class PatchEmbed3D(nn.Module):
    ...
    def __init__(
        ...
        self.proj = nn.Conv3d(
            in_chans,
            embed_dim,
            kernel_size=(temporal_stride, patch_size[0], patch_size[1]),
            stride=(temporal_stride, patch_size[0], patch_size[1]),
        )
    ...
    def forward(self, x):
        ...
        W = x.size(-1)
        x = x.flatten(2).transpose(1, 2)
        return x, T // self.temporal_stride, W
...
class TimeSformer(nn.Module):
    ...
    def __init__(
        ...
        if self.attention_type != "space_only":
            self.time_embed = nn.Parameter(
                torch.zeros(1, num_frames // temporal_stride, embed_dim)
            )
    ...
    def load_weights(self, pretrained_model="", filter_fn=_conv_filter):
        if pretrained_model.startswith("http"):
            state_dict = model_zoo.load_url(
                pretrained_model, progress=False, map_location="cpu"
            )
        ...
        if self.num_patches + 1 != state_dict["pos_embed"].size(1):
            pos_embed = state_dict["pos_embed"]
            cls_pos_embed = pos_embed[0, 0, :].unsqueeze(0).unsqueeze(1)
            other_pos_embed = pos_embed[0, 1:, :].unsqueeze(0).transpose(1, 2)
            new_pos_embed = F.interpolate(
                other_pos_embed, size=(self.num_patches), mode="nearest"
            )
        ...
        if "time_embed" in state_dict and self.num_frames != state_dict[
            "time_embed"
        ].size(1):
            time_embed = state_dict["time_embed"].transpose(1, 2)
            new_time_embed = F.interpolate(
                time_embed, size=(self.num_frames), mode="nearest"
            )
